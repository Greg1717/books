...
# plot the transformed numbers, i.e. after having removed skewness
Ch1AreaTrans_unskewed <- 
        predict(Ch1AreaTrans, segData$AreaCh1)
hist(Ch1AreaTrans_unskewed)
```
...
To visually examine the correlation structure of the data, the corrplot package contains an excellent function of the same name. The function has many options including one that will reorder the variables in a way that reveals clusters of highly correlated predictors.
...
```{r}
highCorr <- caret::findCorrelation(correlations, cutoff = .75)
length(highCorr)
head(highCorr)
...
mainEffects <- caret::dummyVars(~ day + time, data = when)
mainEffects
