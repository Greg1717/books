...
s <- list.files(path = "./kaggle_titanic",pattern = "train", full.names = T)
train <- read.csv(s, stringsAsFactors = F)
s <- list.files(path = "./kaggle_titanic",pattern = "test", full.names = T)
test  <- read.csv(s, stringsAsFactors = F)
...
Create linear model:
```{r}
lm_model_1 <- lm(Survived ~ as.numeric(Pclass) + Sex + Age,
                 data = train)
summary(lm_model_1)
```
...
Predict:
```{r}
lm_model_1_prediction <- predict(lm_model_1, test)
lm_model_1_prediction
```
...
```{r}
library(class)
model_knn <- knn(train = train, test = test, cl = train$Survived)
```
